  ...
  <div>
    ...
    <div class="flex flex-col sm:flex-row gap-3 mb-10">
      <div class="relative flex-1">
        ...
        <input v-model="q" placeholder="Rechercher par titre, lieu, description…" class="ef-input pl-11 h-12" />
      </div>
      <select v-model="sortBy" class="ef-input w-full sm:w-48 h-12">
      ...
      <article v-for="(event, i) in filteredEvents" :key="event.id" class="ef-card overflow-hidden group flex flex-col">
        ...
        <div class="relative h-28 flex items-end p-4"
          ...
          <div class="absolute top-0 left-0 right-0 h-0.5 rounded-t-2xl"
            :style="`background: linear-gradient(90deg, ${accent(i).from}, ${accent(i).to})`"></div>
          ...
          <span v-if="event.isPublished" class="badge-green text-xs">Publié</span>
          <span v-else class="badge-orange text-xs">Brouillon</span>
        </div>
        ...
        <div class="p-5 flex flex-col flex-1">
          ...
          <div class="mt-4 h-1 rounded-full overflow-hidden" style="background:var(--border)">
            <div class="h-full rounded-full transition-all duration-700"
              :style="`width: ${Math.min((event.participantsCount / event.maxParticipants) * 100, 100)}%;
                       background: linear-gradient(90deg, ${accent(i).from}, ${accent(i).to})`"></div>
          </div>
          ...
          <div class="mt-5 pt-4 border-t ef-border flex items-center gap-2 mt-auto">
            ...
            <div v-if="userStore.isOrganizer" class="flex items-center gap-1.5 ml-auto">
              <button @click="eventsStore.togglePublish(event.id)"
                class="text-xs px-2.5 py-1.5 rounded-lg border ef-border text-sub hover:text-main transition"
                style="border-color:var(--border)">
                {{ event.isPublished ? 'Dépublier' : 'Publier' }}
              </button>
              <button @click="handleDelete(event.id)" class="btn-danger text-xs px-2.5 py-1.5">
